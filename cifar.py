...
ffnn_batch_size = 64
cnn_batch_size = 32
ffnn_train_dataloader = DataLoader(training_data, batch_size=ffnn_batch_size)
ffnn_test_dataloader = DataLoader(test_data, batch_size=ffnn_batch_size)
cnn_train_dataloader = DataLoader(training_data, batch_size=cnn_batch_size)
cnn_test_dataloader = DataLoader(test_data, batch_size=cnn_batch_size)
device = "cpu"
...
for t in range(epochs):
  ...
  train(ffnn_train_dataloader, feed_forward_model, loss_fn, ffnn_optimizer)
  test(ffnn_test_dataloader, feed_forward_model, loss_fn, ffnn_losses)
# save the model
torch.save(feed_forward_model.state_dict(), "feed_forward_model.pth") 

for t in range(epochs):
  print(f"=== MODEL 2 - EPOCH {t+1} ===")
  train(cnn_train_dataloader, cnn_model, loss_fn, cnn_optimizer)
  test(cnn_test_dataloader, cnn_model, loss_fn, cnn_losses)
...
def find_image_examples(model, model_name):
  found_correct = False
  ...
  for i in indices:
    ...
    if predicted_class == actual_class and not found_correct:
      ...
      plt.title(f"{model_name} Correctly Predicted: '{predicted_class}', Actual: '{actual_class}'")
      plt.show()
      found_correct = True
    # incorrect example not found yet
    if predicted_class != actual_class and not found_incorrect:
      plt.imshow(x.permute(1, 2, 0))
      plt.title(f"{model_name} Incorrectly Predicted: '{predicted_class}', Actual: '{actual_class}'")
      plt.show()
...
find_image_examples(feed_forward_model, 'Model 1 -')
find_image_examples(cnn_model, 'Model 2 -')
